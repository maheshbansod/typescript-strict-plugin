...
async function findStrictErrors(strictPaths, overrides) {
    if (strictPaths.length === 0) {
        return [];
    }
    const tscErrorMap = await waitWithSpinner_1.waitWithSpinner(() => compile_1.compile(overrides), 'Compiling with strict mode...');
